...
if (ExecutionEnvironment.canUseDOM) {
  // Import Prism from window (set by prism-react-renderer)
  const Prism = (window as any).Prism;

  if (Prism) {
    // Register SSL language
    Prism.languages.ssl = {
      comment: [
        {
          pattern: /\/\*[\s\S]*?\*\//,
