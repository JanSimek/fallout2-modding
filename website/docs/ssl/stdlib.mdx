...
This page documents the built-in functions available in SSL scripts.
...
```ssl
variable hp := critter_stat(dude_obj, STAT_current_hp);
variable str := critter_stat(dude_obj, STAT_strength);
```

### critter_add_trait / critter_rm_trait

Add or remove traits from a critter.
...
Check if critter has skill points.
...
```ssl
move_to(obj, tile_num, elevation);
```
...
```ssl
variable obj := create_object(PID_STIMPAK, tile_num, elevation);
```
