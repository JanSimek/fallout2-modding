...
```ssl
ObjectPtr create_object(int pid, int tileNum, int elev)
```
...
- `tileNum` - Tile number to place the object
- `elev` - Elevation (0-2)
...
```ssl
ObjectPtr create_object_sid(int pid, int tileNum, int elev, int sid)
```
...
Destroys an object. If the object is not the calling object, its script's [`destroy_p_proc`](/ssl/procedures#destroy_p_proc) section will be called.
...
Macro alias for [`has_trait`](./critter#has_trait)`(TRAIT_OBJECT, who, OBJECT_VISIBILITY)`.

:::tip
See [this forum thread](https://www.nma-fallout.com/threads/scripting-issue-checking-invisibility.222380/) for more information about checking object visibility.
