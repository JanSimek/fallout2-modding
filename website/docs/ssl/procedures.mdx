...
**Note:** Use `map_first_run` to check if this is the first time the map has been entered.
...
**Important:** This procedure is **not** called when using `destroy_object(self_obj)` - the object is destroyed before the callback can be invoked.
...
Called when a skill is used on the object. Use `action_being_used` to determine which skill.
...
Called by the `add_timer_event(ObjPtr obj, int time, int fixed_param)` function.
...
Use `fixed_param` in `timed_event_p_proc` to check which event triggered the call. This allows multiple uses of timed events in the same script.
